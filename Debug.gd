...
func plot_single(identifier: String, label: String, value: float, min: float = -1.0, max: float = 1.0) -> void:
    _plotter.add_plot_single(identifier, label, value, min, max)
    
func plot_multi(identifier: String, labels: PoolStringArray, values: PoolRealArray, min: float = -1.0, max: float = 1.0) -> void:
    _plotter.add_plot_multi(identifier, labels, values, min, max)

func _draw_debug_ui() -> void:
    # layout / draw subsystems
    # Console layouts itself